...
Use **Python 3.11** (or 3.12). Avoid 3.14 for now: some dependencies do not have compatible builds yet.
...
5. Ask questions about warranty, returns, shipping, installation, filters, pricing and troubleshooting.
...
Do not upload an API key or `.env` file. The local vector database is excluded by `.gitignore` and is rebuilt automatically.
...
Use `python -m pip ...` and `python -m streamlit ...` instead of `pip` / `streamlit`.
